...
for (i in 1:length(trt_vars)) {
  ...
  mod_temp = lm(trt ~ con, data=temp_dat) # before only
  ...
  out[i, 2:4] <- c(rsq, as.numeric(round(summary(mod_temp)$coefficients[,1], 3)))
  ...
  temp_dat_10 <- data.frame(trt = (dat[,temp_trt]),con = (dat[,temp_con]))
  Max <- max(max(log10(temp_dat_10$con)), max(log10(temp_dat_10$trt)))
  Min <- min(min(log10(temp_dat_10$con)), min(log10(temp_dat_10$trt)))
  #breaks = 10**(1:10)
  
  p <- ggplot(temp_dat_10,aes(x=con,y=trt))+
    geom_point(size=1)+
    stat_smooth(method=lm, se=TRUE, level=.90)+
    scale_x_log10()+
    scale_y_log10()+
    #xlim(limits=(c(Min,Max)))+
    #ylim(limits=(c(Min,Max)))+
    labs(x = paste0('Control site - log ', clean_names[i]), y = paste0('Test site - log ', clean_names[i]))+
    annotate('text', x = -Inf, y = Inf, label = rsq_label, vjust = 1.1, hjust = -0.1, parse = T) +
    #annotate('text', x = -Inf, y = Inf, label = p.label, vjust = 2, hjust = -0.04) +
    theme_bw()+
    theme(aspect.ratio=1,axis.text.x = element_text(size=rel(1.0)))
  p
  # save figure
